...
class InterviewTimelineSerializerADMIN(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = InterviewTimeline
        fields = ('url', 'pk', 'interview', 'location')


class InterviewSerializerADMIN(serializers.HyperlinkedModelSerializer):
    InterviewTimeline = InterviewTimelineSerializerADMIN(many=True)

    class Meta:
        model = Interview
        fields = ('url', 'pk', 'title', 'edit_finish',
                  'is_public', 'InterviewTimeline')
